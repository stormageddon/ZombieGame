    ...
    void ProcessZombieHit()
    {
        ...
        if(Physics.Raycast(shotDirection.position, shotDirection.forward, out hitInfo, 200))
        {
            ...
            if(hitZombie.tag == "Zombie")
            {
                ...
                ZombieController zombie = hitZombie.gameObject.GetComponent<ZombieController>();
                if(Random.Range(0,100) < 50)
                {
                    // GameObject ragDoll = .Find(hitZombie.gameObject;
                    GameObject ragdoll = zombie.ragdoll;
                    GameObject doll = Instantiate(ragdoll, hitZombie.transform.position, hitZombie.transform.rotation);
                    doll.transform.Find("Hips").GetComponent<Rigidbody>().AddForce(shotDirection.forward * 10000);
